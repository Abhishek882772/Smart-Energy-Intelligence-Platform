...
export async function insertStoredFile(file: InsertStoredFile) {
  ...
  const rows = await db.select().from(storedFiles).where(and(eq(storedFiles.userId, file.userId), eq(storedFiles.fileKey, file.fileKey))).limit(1);
  return rows[0];
}

export async function getStoredFileByIdAndUser(id: number, userId: number) {
  const db = await getDb();
  if (!db) return undefined;
  const rows = await db.select().from(storedFiles).where(and(eq(storedFiles.id, id), eq(storedFiles.userId, userId))).limit(1);
  return rows[0];
}

export async function deleteStoredFileByIdAndUser(id: number, userId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
